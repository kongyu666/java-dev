...
/**
 * 演示 Flink 中的基于滚动事件时间窗口的流处理。带时间字段
 */
public class TumblingEventWindowStream02 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 应用水印策略，并进行窗口操作
        SingleOutputStreamOperator<Tuple3<String, Long, String >> operator = streamSource
                .assignTimestampsAndWatermarks(watermarkStrategy)
                .map(new MyMapFunction02())  // 对事件进行转换
                .map(new MapFunction<Tuple2<String, Long>, Tuple3<String, Long, String>>() {
                    @Override
                    public Tuple3<String, Long, String> map(Tuple2<String, Long> value) throws Exception {
                        // 获取当前处理时间
                        return new Tuple3<>(value.f0, value.f1, DateUtil.now());
                    }
                })
                .keyBy(key -> key.f0)  // 根据键分组
                .window(TumblingEventTimeWindows.of(Time.minutes(2)))  // 定义2分钟的事件时间窗口
                .reduce((value1, value2) -> new Tuple3<>(value1.f0, value1.f1 + value2.f1, value1.f2));  // 对窗口内数据进行汇总
